...
function citiesFor(destinationSlug) {
  return citiesByDestination[destinationSlug] ?? [];
}
function allCities() {
  return Object.entries(citiesByDestination).flatMap(
    ([destinationSlug, list]) => (list ?? []).map((city) => ({ destinationSlug, city }))
  );
}
export {
  Search as S,
